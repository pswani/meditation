...
- Native iPhone timer sessions with interval or end bells now start a silent background-audio keepalive so selected cues are more reliable under silent switch, lock screen, and competing-audio conditions; bells-only `custom play` runs use the same approach for delayed completion bells when the recording is unavailable.
- A flaky native XCTest crash in `ShellViewModelTests` was fixed by waiting for async notification scheduling instead of force-unwrapping state before the background task completed.
