...
Goal:
- Address actionable issues found during review or testing for the native iOS foundation milestone.

Rules:
- Stay within the original milestone scope.
- Do not widen into timer runtime, audio playback, `session log`, or backend sync work.
- Fix only validated issues from review or testing.

Required follow-through:
1. Implement only the necessary corrections.
2. Update focused Swift tests if the fixes change expected behavior.
